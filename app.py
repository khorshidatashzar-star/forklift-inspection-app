            ...
            if ans == "⚠️ Issue Found":
                ...
                for j, comp in enumerate(comps):
                    ...
                    photo_source = st.radio(
                        ...
                        horizontal=True,
                        key=f"drv_photo_source_{gi}_{j}"
                    )

                    if photo_source == "📷 Take Photo":
                        pic = st.camera_input(
                            f"{comp} — Take Photo",
                            key=f"drv_camera_{gi}_{j}"
                        )
                    else:
        ...
        if repairing.empty:
            ...
            for _, r in repairing.iterrows():
                with st.expander(
                    ...
                    repair_photo_source = st.radio(
                        "After-repair photo (optional)",
                        ["📤 Upload Photo", "📷 Take Photo"],
                        horizontal=True,
                        key=f"repair_photo_source_{r.id}"
                    )

                    if repair_photo_source == "📷 Take Photo":
                        photo = st.camera_input(
                            "Take After-Repair Photo",
                            key=f"repair_camera_{r.id}"
                        )
                    else:
