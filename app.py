...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS inspections(
      ...
      manager_note TEXT,
      manager_at TEXT,
      out_of_service TEXT DEFAULT 'No',
      maintenance_name TEXT,
      repair_started_at TEXT,
      corrective_action TEXT,
      repair_photo TEXT,
      repair_completed_at TEXT,
      repair_delay_reason TEXT,
      repair_delay_note TEXT,
      repair_eta TEXT,
      next_reminder_at TEXT,
      final_verified_by TEXT,
      final_note TEXT,
      final_at TEXT,
      closed_at TEXT,
      created_at TEXT,
    ...
    if not c.execute("SELECT 1 FROM users WHERE username='itadmin'").fetchone():
        s, h = pw_hash("ChangeMe123!")
        c.execute("""
        INSERT INTO users
        (username,password_salt,password_hash,full_name,role,location,email,active,activated,must_change_password)
        VALUES(?,?,?,?,?,?,?,?,?,?)
        """, ("itadmin", s, h, "IT Administrator", "IT", "ALL", "", 1, 1, 1))

    # V5 lightweight migration for IT location-update requests.
    cols = [r[1] for r in c.execute("PRAGMA table_info(asset_review)").fetchall()]
    if "requested_location" not in cols:
        c.execute("ALTER TABLE asset_review ADD COLUMN requested_location TEXT")


    # Migration for shift-based inspections.
    inspection_cols = [r[1] for r in c.execute("PRAGMA table_info(inspections)").fetchall()]
    if "shift" not in inspection_cols:
        c.execute("ALTER TABLE inspections ADD COLUMN shift TEXT")
...
def equipment():
    ...
    d = pd.read_csv(EQ_FILE, dtype=str).fillna("")
    for col in [
        "serial_number","unit_number","location","brand","description",
        "model","status","building","out_of_service"
    ]:
...
def save_equipment(d):
    ...
    for col in cols:
        if col not in d.columns:
            d[col] = ""
    d[cols].to_csv(EQ_FILE, index=False)

def daily_case_number():
    prefix = f"FI-{date.today():%Y%m%d}-"
    c = conn()
    rows = c.execute(
        "SELECT case_number FROM inspections WHERE case_number LIKE ?",
        (prefix + "%",)
    ).fetchall()
    c.close()
...
def temporary_role_test_access():
    ...
    with st.expander("🧪 Temporary Staff Portal Test Access", expanded=True):
        test_role = st.selectbox(
            "Preview Role",
            ["WH Manager", "Maintenance", "Compliance"],
            key="qa_preview_role"
        )
...
def common_case_report(u):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        humanize_columns(log).to_excel(w, "Daily Inspection Log", index=False)

        issue_rows = query("SELECT * FROM issues")
        refs = day.case_number.tolist()
        humanize_columns(
            issue_rows[issue_rows.case_number.isin(refs)]
        ).to_excel(w, "Issues Reported", index=False)

        pd.DataFrame(summary_rows).to_excel(w, "Shift Summary", index=False)

        if not w.book.worksheets:
            pd.DataFrame({"Message": ["No records available for this report."]}).to_excel(
...
def daily_report_tab(u, key_prefix, all_locations=False):
    ...
    if all_locations:
        locations = ["All"] + sorted(
            [x for x in ins.location.dropna().astype(str).unique() if x]
        )
    ...
    if selected_location != "All":
        day = day[day.location == selected_location]

    if selected_shift != "All Shifts":
        day = day[day["shift"].fillna("") == selected_shift]
    ...
    st.markdown("### Shift Summary")
    base = ins[ins.inspection_date.astype(str) == str(selected_date)].copy()
    if selected_location != "All":
        base = base[base.location == selected_location]
    ...
    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        humanize_columns(log).to_excel(w, "Daily Log", index=False)
        pd.DataFrame(summary_rows).to_excel(w, "Shift Summary", index=False)
...
def case_machine_history_tab(u, key_prefix, all_locations=False):
    ...
    if selected_machine:
        ...
        if mh_all.empty:
            ...
            cols = ["inspection_date","shift","case_number","operator_name","issue_found","status"]
...
def manager_actions(u):
    ...
    with tab_work:
        x = query("SELECT * FROM inspections WHERE location=? ORDER BY id DESC", (u["location"],))
        today_all=x[x.inspection_date.astype(str)==str(date.today())].copy()
        ...
        with st.expander(f"Today's Inspection Status — {len(active)} machine(s)", expanded=False):
            ...
            with pd.ExcelWriter(status_buf, engine="openpyxl") as w:
                machine_status.to_excel(w, "Today's Inspection Status", index=False)
...
def corrective_actions(u):
    ...
    # =========================================================
    with tab_work:
        all_cases = query("SELECT * FROM inspections ORDER BY id DESC")

        repair_cases = all_cases[
            all_cases.status.isin(["Repair Not Started", "Repairing"])
        ].copy()
        ...
        today_all = all_cases[
            all_cases.inspection_date.astype(str) == str(date.today())
        ].copy()
        today_loc = today_all[
            today_all.location == work_location
        ].copy() if work_location else today_all.copy()
    ...
    with tab_reports:
        ...
        if ins.empty:
            ...
            if selected_location != "All":
                base_day = base_day[base_day.location == selected_location]
    ...
    with tab_history:
        ...
        ins = query("SELECT * FROM inspections ORDER BY id DESC")
        issue_cases = ins[
            ins.issue_found.fillna(0).astype(int) == 1
        ].copy()
        ...
        if selected_machine:
            ...
            mh_all = ins[
                ins.serial_number.astype(str).str.upper() == serial.upper()
            ].copy()
...
def it_actions(u):
    st.header("💻 IT")
    st.caption("Staff users • delete assets • approved location updates.")

    st.markdown("### Staff Users")
    users = query("SELECT * FROM users ORDER BY role,full_name")

    for _, r in users.iterrows():
        c1,c2,c3,c4,c5 = st.columns([1.4,2,2,2,1])
        c1.write(r.username)
        c2.write(r.full_name)
        c3.write(r.role)
        c4.write(r.location if r.role=="WH Manager" else "All Locations")
        if str(r.username).lower()=="itadmin":
            c5.caption("Protected")
        elif c5.button("Delete", key=f"delete_user_{r.id}"):
            c=conn()
    ...
    st.markdown("### Add Staff User")
    with st.form("simple_add_user"):
        a,b=st.columns(2)
        full=a.text_input("Full Name")
        username=b.text_input("Username")
        role=st.selectbox("Role",["WH Manager","Maintenance","Compliance","IT"])
        locations=sorted([x for x in equipment().location.unique() if x])
        if role=="WH Manager":
            loc=st.selectbox("Manager Location (set by IT)",locations)
        else:
            loc="ALL"
            st.caption("Access: All Locations")
    ...
    if add:
        if not full.strip() or not username.strip() or not password:
            st.error("Full Name, Username and Temporary Password are required.")
        else:
            salt=secrets.token_hex(16)
            h=hashlib.pbkdf2_hmac("sha256",password.encode(),salt.encode(),220000).hex()
            try:
                c=conn()
                c.execute("""INSERT INTO users(username,password_salt,password_hash,full_name,role,location,email,
                             active,activated,must_change_password)
                             VALUES(?,?,?,?,?,?,?,1,1,0)""",
    ...
    st.markdown("### Location Update Requests")
    req=query("""SELECT * FROM asset_review WHERE status='Waiting IT Location Update' ORDER BY id DESC""")
    eq=equipment()

    if req.empty:
        st.info("No location updates waiting.")

    for _, r in req.iterrows():
        with st.expander(f"{r.case_number} • Serial {r.serial_number} • {r.registered_location} → {r.requested_location}"):
            st.write(f"Compliance note: {r.compliance_note or '—'}")
            if st.button("UPDATE LOCATION", key=f"update_location_{r.id}", type="primary"):
                mask=eq.serial_number.str.upper()==str(r.serial_number).upper()
                old=eq.loc[mask,"location"].iloc[0] if mask.any() else r.registered_location
                eq.loc[mask,"location"]=r.requested_location
                save_equipment(eq)
                c=conn()
                c.execute("""UPDATE asset_review SET status='Completed',updated_at=? WHERE id=?""",(now(),r.id))
                c.execute("""INSERT INTO asset_movements(serial_number,from_location,to_location,case_number,approved_by,changed_at)
                             VALUES(?,?,?,?,?,?)""",(r.serial_number,old,r.requested_location,r.case_number,u["full_name"],now()))
                c.commit();c.close()
                audit(r.case_number,"IT",u["full_name"],"Asset Location Updated",
                      f"{old} -> {r.requested_location}")
                st.rerun()

    st.markdown("### Delete Asset")
    eq=equipment()
    active=eq[~eq.status.str.lower().isin(["deleted"])]
    labels={f"{r.serial_number} • Unit {r.unit_number or '—'} • {r.location} • {r.brand}":idx for idx,r in active.iterrows()}
    if labels:
        choice=st.selectbox("Asset",list(labels.keys()))
        ...
        if st.button("DELETE ASSET",type="primary"):
            if not reason.strip():
                ...
                save_equipment(eq)
                audit("","IT",u["full_name"],"Asset Deleted",f"Serial={serial}; Reason={reason}")
                st.rerun()
