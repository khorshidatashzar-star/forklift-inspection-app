# BUILD: SIMPLE-V40-CONFIDENCE-MACHINE-MATCHING-20260820\n# BUILD: SIMPLE-V39-SMART-MACHINE-DUPLICATE-REVIEW-20260820
# BUILD: SIMPLE-V38-COMPLIANCE-MACHINE-REVIEW-20260820
# BUILD: SIMPLE-V37-SINGLE-PHOTO-CONTROL-20260820
# BUILD: SIMPLE-V36-ALL-PORTALS-SPEED-OPTIMIZED-20260820
# BUILD: SIMPLE-V35-MANAGER-SPEED-FIX-20260820
# BUILD: SIMPLE-V34-CROSS-DEVICE-DUPLICATE-LOCK-20260820
# BUILD: SIMPLE-V33-DUPLICATE-SUBMIT-GUARD-20260820
# BUILD: SIMPLE-V32-SHIFT-REQUIRED-FIX-20260820
# BUILD: SIMPLE-V31-PHOTO-VISIBILITY-CAMERA-20260820
...
def active_locations():
    ...
    if d.empty:
        return []
    active = d[
        ~d.status.astype(str).str.lower().isin(["inactive","retired","deleted"])
    ]
    return sorted([x for x in active.location.dropna().astype(str).unique() if x])
...
def update_equipment_out_of_service(serial_number, out_of_service):
    """Fast single-row update used by Manager decisions."""
    c = conn()
    try:
        c.execute(
            """UPDATE equipment
               SET out_of_service=?, updated_at=NOW()
               WHERE UPPER(TRIM(serial_number))=UPPER(TRIM(?))""",
            (str(out_of_service or "No"), str(serial_number or ""))
        )
        c.commit()
        equipment.clear()
    except Exception:
...
def insert_equipment_row(serial_number, unit_number, location, brand="", description="", model="", status="Active", building="", out_of_service="No"):
    c = conn()
    try:
        c.execute(
            """INSERT INTO equipment(
                   serial_number,unit_number,location,brand,description,model,
                   status,building,out_of_service,created_at,updated_at
               )
               VALUES(?,?,?,?,?,?,?,?,?,NOW(),NOW())
...
def update_equipment_location(serial_number, new_location):
    c = conn()
    try:
        c.execute(
            """UPDATE equipment
               ...
               WHERE UPPER(TRIM(serial_number))=UPPER(TRIM(?))""",
            (str(new_location or ""), str(serial_number or ""))
        )
        c.commit()
        equipment.clear()
    except Exception:
...
def retire_equipment(serial_number):
    c = conn()
    try:
        c.execute(
            """UPDATE equipment
...
@st.cache_data(ttl=120, show_spinner=False)
def case_issue_rows(case_number):
    return query(
        """SELECT id,component,issue_type,driver_note,photo_path,created_at
           FROM issues
           WHERE case_number=?
           ORDER BY id""",
        (case_number,)
...
def render_case_media(case_number, key_prefix="case"):
    """Show issue details plus driver and after-repair photos for a case."""
    issue_rows = case_issue_rows(case_number)

    if not issue_rows.empty:
        display_cols = ["component","issue_type","driver_note","created_at"]
        st.dataframe(
            humanize_columns(
                issue_rows[[c for c in display_cols if c in issue_rows.columns]]
            ),
            hide_index=True,
        ...
        photos = issue_rows[
            issue_rows.photo_path.fillna("").astype(str).str.strip() != ""
        ].copy()

        if not photos.empty:
            st.markdown("**Driver Photo(s)**")
            cols = st.columns(min(3, len(photos)))
            for idx, (_, pr) in enumerate(photos.iterrows()):
                url = signed_photo_url(pr.photo_path, expires_in=3600)
                caption = f"{pr.component} — {pr.issue_type}"
                with cols[idx % len(cols)]:
                    if url:
                        st.image(url, caption=caption, use_container_width=True)
                    else:
                        st.warning(f"Photo unavailable: {caption}")

    repair = query(
        """SELECT repair_photo,corrective_action,maintenance_name,repair_completed_at
           FROM inspections
           WHERE case_number=?
           LIMIT 1""",
        (case_number,)
    ...
    if not repair.empty:
        rr = repair.iloc[0]
        repair_path = str(rr.get("repair_photo") or "").strip()
        if repair_path:
            st.markdown("**After-Repair Photo**")
            url = signed_photo_url(repair_path, expires_in=3600)
            if url:
                st.image(
                    url,
                    caption=f"Repair completed by {rr.get('maintenance_name') or 'Maintenance'}",
                    width=520
                )
            else:
                st.warning("The after-repair photo is currently unavailable.")
...
def already_inspected_fresh(serial, shift):
    """Final submit guard: always query PostgreSQL directly, never cache."""
    if not str(serial or "").strip() or not str(shift or "").strip():
        return None
    ...
    try:
        r = c.execute(
            """SELECT case_number,status
               ...
               ORDER BY id DESC
               LIMIT 1""",
            (str(date.today()), str(serial).strip(), str(shift).strip())
        ).fetchone()
        return dict(r) if r else None
    finally:
        c.close()
...
@st.cache_data(ttl=120, show_spinner=False)
def manager_status_excel_bytes(records):
    from openpyxl import Workbook
    from openpyxl.utils.dataframe import dataframe_to_rows
    ...
    export_df = humanize_columns(df.copy())
    if export_df.empty:
        ws.append(["Message"])
...
def common_case_report(u):
    ...
    if 'hist' in locals() and not hist.empty:
        ...
        st.markdown("**Reported Issue(s) & Photo(s)**")
        render_case_media(
            selected_case,
            f"common_case_{selected_case}"
        )
...
def run_driver():
    ...
    if unlisted:
        ...
        a, b, c, d = st.columns(4)
        serial = a.text_input("Serial Number *", help="Use the machine serial plate — not Model # or Unit #.", key="drv_serial")
        ...
        dup = already_inspected(serial, shift) if shift else None
        ...
        if dup:
            st.warning(
                f"⚠️ This machine was already inspected for the **{shift}** shift today "
                f"under case **{dup['case_number']}**. "
                "You cannot submit another inspection for the same machine and shift."
            )
    ...
    if submit_clicked:
        ...
        if not shift:
            errors.append("**Shift** is required. Please choose Morning, Afternoon, or Overnight.")
        ...
        if not unlisted and shift:
            existing = already_inspected_fresh(serial, shift)
            if existing:
                errors.append(
                    f"Serial **{serial}** was already inspected for the "
                    f"**{shift}** shift today under case **{existing['case_number']}**. "
                    "A second inspection for the same machine and shift cannot be submitted."
                )
        ...
        try:
            ...
            c.rollback()
            existing = already_inspected_fresh(serial, shift)
            case_text = existing["case_number"] if existing else "an existing case"
            st.error(
                f"❌ This inspection was not submitted. Serial **{serial}** was already "
                f"inspected for the **{shift}** shift today under **{case_text}**. "
                "This rule is enforced by the central database across all devices."
            )
            return
...
def case_machine_history_tab(u, key_prefix, all_locations=False):
    ...
    if issue_cases.empty:
        ...
        if not hist.empty:
            ...
            show_case_media = st.checkbox(
                "Show issue details & photo(s)",
                key=f"{key_prefix}_show_case_media_{selected_case}"
            )
            if show_case_media:
                st.markdown("**Reported Issue(s) & Photo(s)**")
                render_case_media(
                    selected_case,
                    f"{key_prefix}_case_{selected_case}"
                )
    ...
    if selected_machine:
        ...
        if mh_issues.empty:
            ...
            selected_ticket = st.selectbox(
                "View Ticket Photo(s) / Details",
                mh_issues.case_number.tolist(),
                index=None,
                placeholder="Choose a ticket",
                key=f"{key_prefix}_machine_ticket_media"
            )
            if selected_ticket:
                render_case_media(
                    selected_ticket,
                    f"{key_prefix}_machine_ticket_{selected_ticket}"
                )
...
def manager_actions(u):
    ...
    with tab_work:
        ...
        with st.expander(f"Today's Inspection Status — {len(active)} machine(s)", expanded=False):
            ...
            status_excel = manager_status_excel_bytes(
                machine_status.to_dict("records")
            )
            st.download_button(
            ...
            for _, r in issue_review.iterrows():
                with st.expander(
                    ...
                    show_media = st.checkbox(
                        "Show issue details & photo(s)",
                        key=f"manager_show_media_{r.id}"
                    )
                    if show_media:
                        st.markdown("**Reported Issue(s) & Photo(s)**")
                        render_case_media(r.case_number, f"manager_task_{r.id}")
                    ...
                    if st.button("SAVE", key=f"mgr_save_{r.id}", type="primary"):
                        if not manager_name.strip():
                            ...
                            update_equipment_out_of_service(
                                r.serial_number,
                                oos
                            )
    ...
    with tab_work:
        ...
        if work_location:
            repair_cases = query(
                """SELECT * FROM inspections
                   WHERE location=?
                     AND status IN ('Repair Not Started','Repairing')
                   ORDER BY id DESC""",
                (work_location,)
            )
        else:
        ...
        waiting = repair_cases[repair_cases.status == "Repair Not Started"].copy() if not repair_cases.empty else pd.DataFrame(columns=repair_cases.columns)
        repairing = repair_cases[repair_cases.status == "Repairing"].copy() if not repair_cases.empty else pd.DataFrame(columns=repair_cases.columns)
        ...
        if waiting.empty:
            ...
            for _, r in waiting.iterrows():
                with st.expander(
                    ...
                    show_media = st.checkbox(
                        "Show issue details & photo(s)",
                        key=f"maintenance_show_media_{r.id}_{r.status}"
                    )
                    if show_media:
                        st.markdown("**Reported Issue(s) & Photo(s)**")
                        render_case_media(r.case_number, f"maintenance_work_{r.id}")
    ...
    with tab_history:
        ...
        if issue_cases.empty:
            ...
            if not hist.empty:
                ...
                render_case_media(
                    selected_case,
                    f"maintenance_history_{selected_case}"
                )
        ...
        if selected_machine:
            ...
            if mh_issues.empty:
                ...
                selected_ticket = st.selectbox(
                    ...
                    index=None,
                    placeholder="Choose a ticket",
                    key="maintenance_machine_ticket_media"
                )
                if selected_ticket:
                    render_case_media(
                        selected_ticket,
                        f"maintenance_machine_ticket_{selected_ticket}"
                    )


def normalize_asset_identifier(value):
    """Normalize serial/unit/model text for safer duplicate detection."""
...
def compliance_actions(u):
    ...
    tab_new, tab_reports, tab_history = st.tabs(
        ["🔎 Machine Review", "📋 Reports", "🕘 Case & Machine History"]
    )
    ...
    with tab_new:
        ...
        if q.empty:
            st.success("No machine reviews are waiting.")
        else:
            for _, r in q.iterrows():
                ...
                with st.expander(title, expanded=False):
                    entered_id = normalize_asset_identifier(r.serial_number)

                    # Primary duplicate check: normalized Serial Number.
                    serial_matches = eq[
                        eq.serial_number.fillna("").map(normalize_asset_identifier) == entered_id
                    ].copy()

                    # Secondary safety check: the driver may have typed a Model or Unit #
                    # into the Serial field. Surface those existing assets before allowing Add.
                    identifier_matches = eq[
                        (
                            eq.model.fillna("").map(normalize_asset_identifier) == entered_id
                        ) |
                        (
                            eq.unit_number.fillna("").map(normalize_asset_identifier) == entered_id
                        )
                    ].copy()
                    ...
                    if matches.empty:
                        if not identifier_matches.empty:
                            unit_matches = identifier_matches[
                                identifier_matches.unit_number.fillna("").map(normalize_asset_identifier) == entered_id
                            ].copy()
                            model_matches = identifier_matches[
                                identifier_matches.model.fillna("").map(normalize_asset_identifier) == entered_id
                            ].copy()

                            if not unit_matches.empty:
                                st.warning(
                                    "⚠️ Possible Existing Machine — the entered value matches an existing **Unit #**."
                                )
                                st.caption(
                                    "Confidence: HIGH, but this is not a confirmed Serial Number match. "
                                    "Verify the physical serial plate before deciding."
                                )
                                candidates = unit_matches
                            else:
                                st.warning(
                                    "⚠️ Possible Match Only — the entered value matches an existing **Model #**."
                                )
                                st.caption(
                                    "Confidence: LOW. Multiple forklifts can share the same model. "
                                    "This does NOT mean the machine is registered."
                                )
                                candidates = model_matches

                            possible_cols = [
                                c for c in [
                                    "serial_number","unit_number","model","brand","location","status"
                                ] if c in candidates.columns
                            ]
                            st.dataframe(
                                humanize_columns(candidates[possible_cols]),
                                hide_index=True,
                                use_container_width=True
                            )

                            st.info(
                                "Verify the forklift's physical Serial Number. Only an exact Serial Number "
                                "match is treated as a confirmed registered machine."
                            )
                            continue

                        st.success(
                            "New machine detected — this serial is not currently registered."
                        )
                        st.caption("Action: Add this machine to the Asset Master.")
                        ...
                        if current == r.found_location:
                            st.success(
                                f"✅ Confirmed Registered Machine — exact Serial Number match at **{current}**."
                            )
                            st.caption(
                                "Action: No asset change is required. Mark this review as completed."
                            )
                            ...
                            st.warning(
                                f"Machine location mismatch — serial is registered at **{current}**, "
                                f"but was reported at **{r.found_location}**."
                            )

                            st.info(
                                "Action: Do not add a duplicate machine. "
                                "Send a Machine Location Update request to IT."
                            )
                            ...
                            if st.button(
                                "SEND MACHINE LOCATION UPDATE TO IT",
                                key=f"send_it_{r.id}",
                                type="primary"
                            ):
                                c = conn()
